...
$ruta = "modulos/aplicacion/formularios/framework/estilos/consultar/estilos.json.php";
$datos = "?transaccion=" . $transaccion . "&buscar=" . $buscar . "&estado=" . $estado . "&clase=" . $clase;
$url = $ruta . $datos;
$itable="itableestilos";
?>
<script type="text/javascript">
  ...
  function explorar_Click<?php echo($transaccion); ?>(button, grid) {var indices = <?php echo($itable);?>.getSelectedIndices();if (indices.length == 0) {return;}for (var i = 0; i < indices.length; i++) {var estilo= <?php echo($itable);?>.getDataByRow(indices[i]).estilo;MUI.Aplicacion_Framework_Clase_Consultar(estilo);}}
  function editar_Click<?php echo($transaccion); ?>(button, grid) {var indices = <?php echo($itable);?>.getSelectedIndices();if (indices.length == 0) {return;}for (var i = 0; i < indices.length; i++) {var estilo= <?php echo($itable);?>.getDataByRow(indices[i]).estilo;MUI.Aplicacion_Framework_Estilo_Actualizar(estilo);}}
  function crear_Click<?php echo($transaccion); ?>(button, grid) {MUI.Aplicacion_Framework_Estilo_Crear('<?php echo($clase); ?>');}
  function eliminar_Click<?php echo($transaccion); ?>(button, grid) {var indices = <?php echo($itable);?>.getSelectedIndices();if (indices.length == 0) {return;}for (var i = 0; i < indices.length; i++) {var estilo= <?php echo($itable);?>.getDataByRow(indices[i]).estilo;MUI.Aplicacion_Framework_Estilo_Eliminar(estilo);}}

  var cmu = [
    {header: "Estilo", dataIndex: 'estilo', dataType: 'string', align: 'center', width: 90},
    {header: "Clase", dataIndex: 'clase', dataType: 'string', align: 'center', width: 90},
    {header: "Sub-Clase", dataIndex: 'subclase', dataType: 'string', width: 250},
